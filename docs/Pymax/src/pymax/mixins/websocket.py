...
class WebSocketMixin(ClientProtocol):
    ...
    async def _outgoing_loop(self) -> None:
        try:
            while self.is_connected:
                try:
                    ...
                    try:
                        ...
                        self.logger.debug("Outgoing loop cancelled during queue get")
                        break
                    
                    if message is None:
                        # Sentinel value для завершения цикла
                        break
                    if not message:
                        continue
                    ...
                    self.logger.debug("Outgoing loop cancelled")
                    break
                except Exception:
            ...
            self.logger.exception("Fatal error in outgoing loop")
