...
## Тестирование с использованием мок-сервера

Для запуска тестов с мок-сервером требуется, чтобы сервер был запущен отдельно (например, в CI/CD конвейере). Адреса HTTP и WebSocket интерфейсов передаются через настройки pytest и используются только как URL, без прямого управления бинарником со стороны Python-тестов.

В файле `pyproject.toml` задаются параметры по умолчанию:

- `mockserver_http_url` — базовый HTTP URL мок-сервера (например, `http://127.0.0.1:8080`)
- `mockserver_ws_url` — WebSocket URL для подключения клиента (например, `ws://127.0.0.1:8080/`)

Тесты читают эти значения через фикстуры `mockserver_http_url` и `mockserver_ws_url` из `conftest.py` и обращаются к серверу напрямую с помощью HTTP-запросов и WebSocket-подключений.
